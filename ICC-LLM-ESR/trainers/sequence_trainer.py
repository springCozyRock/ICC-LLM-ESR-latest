...
class SeqTrainer(Trainer):
    ...
    def test_group(self):
        ...
        # 2. 序列长度5分组指标（接收多topk结果）
        hr_len_list, ndcg_len_list, count_len = metric_len_5group(
            pred_rank.detach().cpu().numpy(), 
            seq_len.detach().cpu().numpy(), 
            thresholds=[5, 10, 15, 20],  # 5分组阈值
            topk=[10, 20]  # 需计算的topk
        )
        # 打印长度分组结果（按topk分开）
        self.logger.info("\nUser Group Performance (by Sequence Length):")
        topk_list = [10, 20]
        for k_idx, (k, hr_len, ndcg_len) in enumerate(zip(topk_list, hr_len_list, ndcg_len_list)):
            self.logger.info(f"\t Top-{k} Results:")
            for group_idx, (hr, ndcg) in enumerate(zip(hr_len, ndcg_len)):
                self.logger.info(f"\t\t Group {group_idx}: HR@{k} = {hr:.4f}, NDCG@{k} = {ndcg:.4f}")

        # 3. 物品流行度5分组指标（接收多topk结果）
        hr_pop_list, ndcg_pop_list, count_pop = metric_pop_5group(
            pred_rank.detach().cpu().numpy(), 
            self.item_pop, 
            target_items.detach().cpu().numpy(), 
            thresholds=[10, 30, 60, 100],  # 5分组阈值
            topk=[10, 20]  # 需计算的topk
        )
        # 打印流行度分组结果（按topk分开）
        self.logger.info("\nItem Group Performance (by Popularity):")
        for k_idx, (k, hr_pop, ndcg_pop) in enumerate(zip(topk_list, hr_pop_list, ndcg_pop_list)):
            self.logger.info(f"\t Top-{k} Results:")
            for group_idx, (hr, ndcg) in enumerate(zip(hr_pop, ndcg_pop)):
                self.logger.info(f"\t\t Group {group_idx}: HR@{k} = {hr:.4f}, NDCG@{k} = {ndcg:.4f}")
